(function () {
  ...
  function renderPackage(root, pkg) {
    ...
    notice.textContent = 'Browser-local Deploy render from loaded package JSON.\nControls are interactive, but no submit, storage, service, or federation action is active.';
  ...
  function textareaRowsForLayout(layoutItem) {
    ...
    return Math.max(3, height || 3);
  }

  function textareaMinHeightForLayout(layoutItem, gridSize) {
    const height = layoutItem && parseInt(layoutItem.height, 10);
    const size = parseInt(gridSize, 10) || 24;
    const gridHeight = Math.max(3, height || 3) * size;
    return Math.max(72, gridHeight - 56) + 'px';
  }
