(function () {
  ...
  function defaultRow(object) {
    if (object.type === 'textarea') {
      return compactTextareaRow('Default', 'default', object.default || '', false, 3);
    }
  ...
  function compactTextareaRow(label, property, value, readonly, rows) {
    const id = 'webforms-prop-' + property.replace(/\./g, '-');
    return [
      '<div class="webforms-property-row mb-1">',
      '<label class="small mb-0" for="' + ns.escapeHtml(id) + '">' + ns.escapeHtml(label) + '</label>',
      '<textarea id="' + ns.escapeHtml(id) + '" class="form-control form-control-sm" rows="' + rows + '" data-webforms-property="' + ns.escapeHtml(property) + '"' + (readonly ? ' readonly="readonly"' : '') + '>',
      ns.escapeHtml(value),
      '</textarea>',
      '</div>'
    ].join('');
