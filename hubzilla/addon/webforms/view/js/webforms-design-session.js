(function () {
  ...
  ns.persistDraft = function (draft) {
    ...
    try {
      ...
      const routeFormId = routeFormIdForDraft(draft);

      window.sessionStorage.setItem(storageKeyForForm(draft.form.id), payload);

      if (routeFormId && routeFormId !== draft.form.id) {
        window.sessionStorage.setItem(storageKeyForForm(routeFormId), payload);
      }
  ...
  function isValidSessionDraft(draft, formId) {
    if (!draft || draft.schema !== 'hubzilla.webforms.designDraft' || draft.version !== ns.VERSION || !draft.form || !Array.isArray(draft.objects)) {
      return false;
    }

    if (draft.form.id === formId) {
      return true;
    }

    return routeFormIdForDraft(draft) === formId;
  }
  ...
  function updateRuntimeState(draft, runtime) {
    ...
    draft.access = { mode: access, public_local_only: access === 'public' };
    draft.design.active_tab = runtime.dataset.webformsDesignTab || 'grid';
    draft.design.route_form_id = routeFormId;
  }

  function routeFormIdForDraft(draft) {
    return draft && draft.design && draft.design.route_form_id ? draft.design.route_form_id : '';
  }
